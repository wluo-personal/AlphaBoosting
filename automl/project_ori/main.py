...
if autolib_dir not in sys.path:
    sys.path.append(autolib_dir)
print('autolib_dir in syspath(check if it\'s right): ', sys.path[-1])
...
def params_gen(model='lgb'):
    ...
    if model == 'svc':
        ...
        params = {
            'objective': 'binary',
            'boosting': 'gbdt',
            'num_boost_round': 30000, # ignored in params. extract it and put it in input arguments in train or cv explicitly
                                   # seems to work fine as the upper limit when combined with early_stopping_round
            'learning_rate': np.random.choice([0.015]),
            'max_bin': np.random.choice([511, 255]),
            'min_data_in_leaf': np.random.choice([20, 120, 220, 320, 420]),
            'num_leaves': np.random.randint(16, 40),
...
#             'min_split_gain': np.random.randint(2, 60)/1000,
            ...
            'feature_fraction': np.random.randint(5,35)/100,
            'feature_fraction_seed': seed,
            'early_stopping_round':250,#np.random.randint(150,300),
            'bagging_fraction': np.random.randint(75,101)/100, #Randomly select part of data
            'bagging_seed': seed,
            'scale_pos_weight': 1,#np.random.choice([1, 5]),#np.random.randint(10,50)/10,
            'lambda_l1': np.random.choice([0.1,0.5,1]),#np.random.randint(2,50)/100,
            'lambda_l2': np.random.choice([0.01, 0.4, 1]),#np.random.randint(2,20)/10,
            'verbose_eval': 500,
            'metric': 'auc',
            'verbose': -1
        }
...
#             'gamma': 0,
        ...
        params = {
            ...
            'best_epoch': 200,
            'patience': 5,  #TODO: not working. fix it
            'categorical_feature': [],
...
if __name__ == '__main__':
    ...
    AlphaBoosting(automl_config_file, features_to_gen, params_gen, fixseed=41, auto_sub_func=None)  # kaggle_auto_sub)
